...
import { initDebugLog, closeDebugLog, logRound, logMessagesSent, logResponse, logToolCall, logToolResult, logCompaction, logError, logSessionStart } from "./ca_log.ts";
...
function finishAssistant(rounds, usage) {
  ...
  if (usage) {
    document.getElementById("sb-stats").innerHTML =
      \`Tokens: <span style="color:#58a6ff">\${usage.totalTokens.toLocaleString()}</span> this round &nbsp;<span style="color:#484f58">(in:\${usage.promptTokens.toLocaleString()} out:\${usage.completionTokens.toLocaleString()})</span>\`;
  }
}
...
function addToolCall(id, name, args) {
  ...
  block.innerHTML = \`<div class="tool-header" onclick="this.parentElement.classList.toggle('expanded')"><span class="icon">\${icon}</span><span class="name">\${name}</span><span class="args">\${escapeHtml(argsStr)}</span><span class="toggle">▶</span></div><div class="tool-result">running…</div>\`;
